...
volunteer_sign_up <- class_schedule |> 
  ...
  mutate(Volunteer = ifelse(instructor == "David Liu", "Loren", ""),
day = paste(day, "Class")) |> 
...
cleanup <- data.frame(day = c("Clean up (2x volunteers on agreed upon date after last class)"),
time = c("TBA"),
date = last_class)

pickup <- data.frame(day = c("Pick up", "Pick up"),
time = c("14:00 - 16:00", "14:00 - 16:00"),
date = pick_up_date(last_class))

ssid <-  as_sheets_id(volunteer_sheet)
sheet_write(bind_rows(volunteer_sign_up, cleanup, pickup), ss = ssid, sheet = session)
